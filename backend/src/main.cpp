...
static void print_result(const std::string &label,
  ...
  std::println("  Rendement annualise : {:.2f} %",
...
static UserInstruction read_user_instruction() {
  ...
  if (instruction.rebalance_period_days == 0)
    throw std::invalid_argument(
        "La periode de reequilibrage doit etre d'au moins 1 jour.");
...
int main() {
  try {
    ...
    BacktestResult gave_result =
        backtest.execute_backtest(gave_portfolio, gave_strategy);

    BuyAndHoldStrategy bah_strategy;
    Portfolio bah_portfolio(instruction.initial_cash);
    BacktestResult bah_result =
        backtest.execute_backtest(bah_portfolio, bah_strategy);
