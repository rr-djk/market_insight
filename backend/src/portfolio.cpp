...
Portfolio::Portfolio(double cash_value)
    : initial_cash(cash_value), available_cash(cash_value) {}
...
bool Portfolio::buy_stock(const std::string &symbol, unsigned int quantity,
  ...
  transaction_history.push_back(
      {date, symbol, OrderType::BUY, quantity, unit_price});
  return true;
}
...
bool Portfolio::sell_stock(const std::string &symbol, unsigned int quantity,
  ...
  if (it == stock_positions.end() || it->second < quantity)
    return false;
  ...
  if (it->second == 0)
    stock_positions.erase(it);
  transaction_history.push_back(
      {date, symbol, OrderType::SELL, quantity, unit_price});
  return true;
}

double Portfolio::compute_total_value(
...
double Portfolio::get_initial_cash() const { return initial_cash; }

double Portfolio::get_available_cash() const { return available_cash; }

const std::unordered_map<std::string, unsigned int> &
Portfolio::get_stock_positions() const {
